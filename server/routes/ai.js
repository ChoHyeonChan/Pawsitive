...
router.post('/recommend-breed', async (req, res) => {
  try {
    ...
    function buildFallbackRecommendation() {
      const selected = candidates.slice(0, recommendCount);
      const levelText = { low: '낮음', medium: '보통', high: '높음' };
      ...
      return {
        ...
        recommendations: selected.map(b => ({
          ...
          name: b.name,
          nameEn: b.nameEn || '',
          matchScore: b._matchScore || 70,
          reason: `${b.name}은(는) 선택한 생활 조건과 데이터 기준으로 잘 맞는 후보예요. ${b.personality || '성향 정보가 안정적인 품종입니다.'}`,
          pros: [
            `${levelText[b.exerciseLevel] || '보통'} 활동량`,
            `${levelText[b.groomingLevel] || '보통'} 관리 난이도`,
            `${levelText[b.trainability] || '보통'} 훈련 적응`
          ],
          cons: [
            b.barkingLevel === 'high' ? '짖음 관리 필요' : '생활 리듬 적응 필요',
            b.groomingLevel === 'high' ? '정기 미용 필요' : '꾸준한 기본 관리 필요'
          ],
          tip: b.exerciseTips || b.groomingTips || '입양 전 실제 생활 패턴과 산책 시간을 한 번 더 맞춰보세요.'
        })),
        summary: 'AI 응답 형식이 흔들려 데이터 기반 추천으로 정리했어요. 선택한 조건과 가까운 품종부터 보여드립니다.',
...
반드시 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{
  ...
  "summary": "전체 추천 요약 (1~2문장)"
    ...
    try {
      ...
      console.log('[품종추천] JSON 파싱 실패, 데이터 기반 추천으로 대체:', e.message);
      return res.json(buildFallbackRecommendation());
    }
